...
export async function postConvertQuoteIntoBooking(quoteId, payload) {
  return withAuthorization(async () => {
    return axiosInstance.post(
      `api/users/quotes/convertToBooking/${quoteId}`,
      payload
    );
  });
}
...
export async function rejectQuoteAPI(messageId, payload) {
  return withAuthorization(async () => {
    return axiosInstance.put(
      `/api/chat/reject-qutation/${messageId}`,
      payload
    );
  });
}